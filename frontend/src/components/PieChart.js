...
const PieChart = ({
  ...
  labelField,
  tooltipFunction,
  colorScheme = ["#1E3A8A", "#2563EB", "#60A5FA", "#93C5FD", "#F3F4F6", "#6B7280", "#111827"],
  w = 550,
  h = 350
}) => {
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    const tooltip = d3.select("body").append("div").attr("class", "tooltip").style("position", "absolute")
      .style("background", "rgba(0, 0, 0, 0.7)").style("color", "white").style("padding", "5px 10px")
      .style("border-radius", "5px").style("opacity", 0).style("pointer-events", "none").style("font-size", "12px");

    arcs.append("path").attr("d", arc).attr("fill", (d) => color(d.data[labelField]))
      .on("mouseover", function (event, d) {
        d3.select(this).transition().duration(300).attr("d", arcHover);
        tooltip.transition().duration(300).style("opacity", 1);
        tooltip.html(tooltipFunction(d.data))
    ...
    arcs.append("text").attr("transform", (d) => `translate(${arc.centroid(d)})`).attr("text-anchor", "middle")
      .text((d) => d.data[labelField]).style("fill", "white").style("font-size", "12px");
